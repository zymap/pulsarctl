...
import (
	...
	"github.com/streamnative/pulsarctl/pkg/bookkeeper/data"
...
type Ledger interface {
	...
	Get(int64) (map[int64]data.LedgerMetadata, error)
...
func (c *ledger) Get(ledgerID int64) (map[int64]data.LedgerMetadata, error) {
	endpoint := c.client.endpoint(c.basePath, "metadata")
	c.params["ledger_id"] = strconv.FormatInt(ledgerID, 10)
	var metadata map[int64]data.LedgerMetadata
	_, err := c.request.GetWithQueryParams(endpoint, &metadata, c.params, true)
	return metadata, err
}
